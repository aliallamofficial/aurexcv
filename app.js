...
function handleCVCreation() {
    const maxAllowedPerDay = 5; // الحد الأقصى المتفق عليه
    const today = new Date().toDateString(); // تاريخ اليوم من ساعة الهاتف
    
    let savedDate = localStorage.getItem('cv_creation_date');
    let creationCount = parseInt(localStorage.getItem('cv_creation_count')) || 0;

    // إذا تغير التاريخ عن التاريخ المحفوظ، قم بتصفير العداد ليوم جديد تلقائياً
    if (savedDate !== today) {
...
// حدث الضغط على زر التحسين ومعالجة النصوص بالذكاء الاصطناعي البشري
document.getElementById('optimizeBtn').addEventListener('click', async () => {
    ...
    // استدعاء فحص الـ 5 مرات أولاً؛ في حال الحظر يوقف دالة المعالجة فوراً
    ...
    // بناء نص الأوامر (Prompt) بدقة متناهية وبأسلوب يمنع النمطية الآلية
    let promptMessage = "";
    if (selectedLang === 'ar') {
    ...
    try {
        ...
        // الحل الاحتياطي الدقيق والمنسق بشرياً في حال انقطاع الشبكة
        let fallbackHTML = "";
        if (selectedLang === 'ar') {
            fallbackHTML = `
...
// 3. ميزة تحميل السيرة الذاتية وطباعتها بصيغة PDF الاحترافية والمدمجة بنظام الهاتف
document.getElementById('downloadPdfBtn').addEventListener('click', () => {
    const printContent = document.getElementById('cvTemplateArea').innerHTML;
    const originalContent = document.body.innerHTML;

    // استبدال الشاشة مؤقتاً لعرض محتوى السيرة الذاتية فقط لطباعة نظيفة بدون حقول الإدخال
